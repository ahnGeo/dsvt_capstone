...
def train(args):
    ...
    if args.dataset == "diving48" :
        dataset_train = Diving48(cfg=cfg, mode="train", num_retries=10)
        dataset_val = Diving48(cfg=cfg, mode="val", num_retries=10)
        cfg.TEST.NUM_SPATIAL_CROPS=3
        cfg.NUM_ENSEMBLE_VIEWS=1
        multi_crop_val=Diving48(cfg=cfg, mode="val", num_retries=10)
    ...
    # Build the video model and print model statistics.
    model = get_vit_base_patch16_224(cfg=cfg, no_head=False)     #^ head = linear classifier
