...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, 
        ...
        if use_learnable_pos_emb:
            self.pos_embed = nn.Parameter(torch.zeros(1, num_patches, embed_dim))   #^ no cls_token -> 196
        else:
        ...
        self.norm = nn.Identity() if use_mean_pooling else norm_layer(embed_dim)    #^ mean pooling -> norm X
...
def get_vmae_vit_base_patch16_224(cfg, no_head=False, pretrained=False, **kwargs):
    patch_size = 16
    vit = VisionTransformer(img_size=cfg.DATA.TRAIN_CROP_SIZE, num_classes=cfg.MODEL.NUM_CLASSES,
                            ...
                            attn_drop_rate=0., drop_path_rate=0.1, all_frames=cfg.DATA.NUM_FRAMES,
                            **kwargs)
